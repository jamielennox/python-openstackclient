...
class OSCDefaultAuthPlugin(auth.BaseAuthPlugin):
    ...
    def get_auth_ref(self, session, **kwargs):
        # NOTE(jamielennox): This function shouldn't be required. Unfortunately
        # there are still calls that depend upon it so we should do our best.
        try:
            get_auth_ref = self._plugin.get_auth_ref
        except AttributeError:
            return None

        return get_auth_ref(session, **kwargs)
